...
async def get_groups(session: AsyncSession, user_id: uuid.UUID) -> list[AssetGroupRead]:
    ...
    # Bulk pre-compute the per-group rollups in a single pass instead of
    # the previous loop that fired ~300 sequential queries on a 140-asset
    # portfolio (one SELECT per group + one latest-value SELECT per
    # asset + two FX rate SELECTs per non-primary-currency asset). The
    # /api/asset-groups handler was clocking 25-28s under HTTP load
    # because of this — now it's a constant ~3 round-trips.
    rollups = await _bulk_rollup(session, user_id, primary)

    # Pre-load institution names for all connection_ids in one query.
    conn_ids = [g.connection_id for g in groups if g.connection_id is not None]
    inst_by_conn: dict[uuid.UUID, str] = {}
    if conn_ids:
        from app.models.bank_connection import BankConnection
        conn_rows = (await session.execute(
            select(BankConnection.id, BankConnection.institution_name)
            .where(BankConnection.id.in_(conn_ids))
        )).all()
        for cid, name in conn_rows:
            if name:
                inst_by_conn[cid] = name

    reads = []
    for g in groups:
        count, cv, cvp = rollups.get(g.id, (0, Decimal("0"), Decimal("0")))
        ...
        if g.source != "manual" and count == 0:
            continue
        institution = inst_by_conn.get(g.connection_id) if g.connection_id else None
...
async def _bulk_rollup(
    ...
    """Return {group_id: (count, current_value, current_value_primary)}
    for every wallet of `user_id` using exactly 3 queries:

    1. All non-archived, non-sold assets in the user's portfolio.
    2. Latest AssetValue per asset (DISTINCT ON).
    3. Today's FX rate per foreign currency in the portfolio.

    Matches `_rollup`'s semantics: skip assets without any value, fall
    back to purchase_price when value history is empty, convert FX into
    the user's primary currency via the latest rate.
    """
    ...
    latest_by_asset: dict[uuid.UUID, Decimal] = {
        aid: Decimal(str(amt)) for aid, amt in latest_rows
    }

    foreign_ccys = {a.currency for a in asset_rows if a.currency != primary_currency}
    rate_by_ccy: dict[str, Decimal] = {}
    for ccy in foreign_ccys:
        # Single get_rate call per currency, cached in-process.
        rate_by_ccy[ccy] = await get_rate(session, ccy, primary_currency)

    out: dict[uuid.UUID, tuple[int, Decimal, Decimal]] = {}
    for asset in asset_rows:
        ...
        if latest is None:
            latest = (
                ...
                if asset.purchase_price is not None else None
            )
        if latest is None:
            continue
        count, cv, cvp = out.get(asset.group_id, (0, Decimal("0"), Decimal("0")))
        count += 1
        cv += latest
        if asset.currency == primary_currency:
            cvp += latest
            ...
            cvp += latest * rate if rate is not None else latest
        out[asset.group_id] = (count, cv, cvp)
    return out
