...
async def import_orders(
    ...
    *,
    ...
    for order in accepted:
        ...
        # O historico do fork usa `type` maiusculo (dez eventos, nao dois),
        # `qty`/`fees`, exige `user_id` e guarda em `value` o caixa que
        # efetivamente saiu -- e dele que o Modified Dietz se alimenta.
        order_qty = Decimal(str(order.quantity))
        order_price = Decimal(str(order.price))
        order_fee = order.fee or Decimal('0')
        session.add(AssetTransaction(
            user_id=user_id,
            asset_id=asset.id,
            workspace_id=workspace_id,
            type='BUY' if order.kind == 'buy' else 'SELL',
            qty=order_qty,
            price=order_price,
            value=(order_qty * order_price).quantize(Decimal('0.01')),
            fees=order_fee,
            date=order.date,
            source='import',
