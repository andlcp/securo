...
async def test_undo_keeps_a_holding_that_has_other_orders_and_recomputes_it(
    ...
    # Campos do fork: `type` maiusculo, `qty`/`fees`, `user_id` obrigatorio
    # e `value` com o caixa que saiu -- e dele que o Modified Dietz vive.
    session.add(AssetTransaction(
        user_id=test_user.id,
        asset_id=asset.id, workspace_id=test_workspace.id, type="BUY",
        qty=Decimal("4"), price=Decimal("200.00"), value=Decimal("800.00"),
        fees=Decimal("0"), date=date(2026, 3, 1), source="manual",
    ))
    await session.commit()
...
async def test_undo_leaves_a_pre_existing_holding_alone(
    ...
    session.add(AssetTransaction(
        user_id=test_user.id,
        asset_id=existing.id, workspace_id=test_workspace.id, type="BUY",
        qty=Decimal("2"), price=Decimal("50.00"), value=Decimal("100.00"),
        fees=Decimal("0"), date=date(2025, 1, 1), source="manual",
    ))
    await session.commit()
